...
**👤 你的 review**：
> 這部分無法使用，修改配置，需要可以直接在畫面上達成以下目的
> 1. 修改 AI 引擎與其參數，尤其是跟寫作相關的參數
> 2. 需要顯示 
> 	1. 前一章節，若無可留空白
> 	2. 到目前為止的劇情摘要，需要同步 Story_status
> 	3. 各角色在該章節的角色狀態 Character_status 
> 	4. 本章劇情大綱 (文字輸入) 
> 	5. 本章寫作需求 (文字輸入)
> 3. 按下生成，先生最後要給 AI 的完整提示詞，給使用者最後編輯，才送出給 AI ，若不滿意可以退回
> 4. 生成的章節內容如果被保留，給AI 的提示詞要存在 ChapterPromptHistory.md 之中，
...
**👤 你的 review**：
> 角色完整描述需要可以手動輸入修改，並以手動修改為準，AI統整功能，無法動作。即使有 AI 統整，在使用者沒有同意以前，不可以蓋掉原先手動輸入的內容。使用者資訊要存在 SQL-Lite 之中
...
**👤 你的 review**：
> (需求)Ｇemini, OpenAI, Anthropic 等應該要可以選模型，不要用輸入的
> (Bug)API token，按了顯示或是隱藏會清除資料愈
---
...
**👤 你的 review**：
> 1. P1. 模型種類要根據提供者可以提供的種類，使用下拉選單
> 2. P1. API Key 預設顯示
> 3. P0. 外部模型要加系統提示詞，如「你是一個繁體中文的小說寫作者，擅長描寫男女情愛細節」
...
> (P2) 角色照片管理
> (P2) 每章角色描寫
> (P0)AI 統整只是輔助 Option ，重點的完整角色描述並沒有出現給使用者填寫，這主要是文字內容
> (P1)填寫內容需要灰色內容輔助輸入
> (P1) AI 統整功能失敗無法點擊產生角色描述
> 
