...
> Status: `Ready`（PM Round 1 拍板 2026-05-18；prompt.md kind 兩 path 修訂核准）
...
## prompt.md 渲染（M6 修訂 — 支援 structured path）

依 `PromptSnapshot.kind` 分支渲染。

### Frontmatter（兩 path 共用）
...
agentVersion: v0.2
model: xiaohuangwen:latest
chapterNumber: 1
chapterTitle: 梅雨初晴
contextHash: 3f9a2b...
kind: structured                  # M6：messages | structured
userEdited: true                  # M6：messages path 才填；structured path 留 null
participants: [蘇晴, 林書言]
outline: |
  本章從蘇晴推開書店木門的雨後場景開始……
requirements: |
...
### Structured path 段（M6 新增）

```markdown
## 結構化生成欄位（M6）

> 使用 `xiaohuangwen:latest` 結構化 API 生成；不存在 promptText 概念。
...
### requirements
<完整 requirements 欄位內容>

### pre_summary
<完整 pre_summary 欄位內容>

### prev_segment
<完整 prev_segment 欄位內容（取最後 2000 codepoint）>
```
...
- 串流耗時: 47s
- 輸入 token: 2310                    # messages path 才填
- 輸出 token: 1842                    # messages path 才填
- 字數: 1842                          # structured path 才填（xiaohuangwen 計費單位）
<!-- /adopt-marker -->
```

**M6 變更要點**：
- frontmatter 加 `kind`（messages | structured）；`userEdited` 在 structured path 留 null
- 兩 path 各自一段 markdown（`## 使用者送出的 Prompt` vs `## 結構化生成欄位`）
- 模型回應 metadata 依 path 決定填 token 或字數
...
- `2026-05-17`（M6 SA-2，待 PM Round 1 拍板轉 Ready）：
  - prompt.md 渲染分 messages / structured 兩 path（依 `PromptSnapshot.kind`）
  - frontmatter 加 `kind`；模型回應 metadata 依 path 填「token」或「字數」
  - contextHash 計算公式**不變**（仍以 ChapterContext 哈希為基底；`kind` 不影響 hash），DRAFT_STALE 行為自然繼承
  - 採用事務步驟 5 / 7 邏輯不變（PromptSnapshot 已涵蓋兩 path 共用欄位）
